<script>
    ...
    (function($) {
        ...
        function createCalendar(month, year, options) {
            ...
            let i = 0;
            let fechas_php = [<?= !empty($fechas['fecha_texto']) ? $fechas['fecha_texto'] : '' ?>];
           
            f = [];
            $.each(fechas_php, function(index, value) {
                f.push(value);
            });
         
            let mes_actual_real = currentMonth + 1;
            if (mes_actual_real < 10) {
                mes_actual_real = '0' + mes_actual_real;
            }
            // console.log(mes_actual_real)
            while (daysLeft) {
                calendar[i] = [];
            ...
            if (currentMonth === mes_actual && (parseInt(year) == parseInt(anio_actual))) {
                ...
                $('td', frame).filter(function() {
                    if (((parseInt($(this).text()) < parseInt(dia_actual)) && (parseInt(year) <= parseInt(anio_actual)))) {
                        ...
                        if ((parseInt($(this).text()) >= parseInt(dia_actual)) && parseInt(currentMonth) === parseInt(mes_actual) && (parseInt(year) == parseInt(anio_actual))) {
                            ...
                            let dia_actual_txt = dia + '-' + mes_actual_real + '-' + anio_actual;
                           
                            // console.log(dia_actual_txt)
                            if ($.inArray(dia_actual_txt, f) >= 0) {
                ...
                $('td', frame).filter(function() {
                    ...
                    if (resultado == false || resultado == 'false') {
                        ...
                        dia_str = parseInt($(this).text()) < 10 ? '0'+$(this).text() : $(this).text();
                        let dia_actual_txt = dia_str + '-' + mes_actual_real + '-' + anio_actual;
                        // console.log(dia_actual_txt)
                        if ($.inArray(dia_actual_txt, f) >= 0) {
                            $($(this), frame).css('text-decoration', 'underline');
                            $($(this), frame).css('text-decoration-color', 'red');
