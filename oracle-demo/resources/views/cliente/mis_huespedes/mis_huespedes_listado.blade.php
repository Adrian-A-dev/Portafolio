    ...
    <div class="card-box mb-30">
        ...
        <div class="pb-20 table-responsive">
            <table class="data-table table stripe nowrap text-center" id="tabla">
                <thead>
                    <tr>
                        <th class="table-plus datatable-nosort">Huésped</th>
                        <th>Reservas</th>
                ...
                <tbody>
                    ...
                    <tr id="fila_<?=  $huesped->id_huesped ?>">
                        <td class="table-plus text-left "><input type="hidden" value="<?=  strUpper($huesped->nombre . ' ' . $huesped->apellido) ?>" id="nombre_<?= $huesped->id_huesped ?>">
                            <b class="ml-4">Nombre:</b> <?= !empty($huesped->nombre) ? strUpper($huesped->nombre . ' ' . $huesped->apellido) : "<span class='text-danger'>Sin información</span>" ?> <br>
                            <b class="ml-4">DNI:</b> <?= !empty($huesped->dni) ? formateaRut($huesped->dni) : "<span class='text-danger'>Sin información</span>" ?><br>
                            <b class="ml-4">Email:</b> <?= !empty($huesped->email) ? $huesped->email : "<span class='text-danger'>Sin información</span>" ?><br>
                            <b class="ml-4">Celular:</b> <?= !empty($huesped->celular) ? $huesped->celular : "<span class='text-danger'>Sin información</span>" ?>
                        </td>
                        <td>{{ $huesped->cantidad > 0 ? formatear_miles($huesped->cantidad) : 0; }}</td>
                        <td>{{ordenar_fechaHoraMinutoHumano($huesped->created_at) }}</td>
                        <td>
                            <div class="dropdown">
                                ...
                                <button type="button" id="<?=  $huesped->id_huesped ?>" class="dropdown-item btn_deleted" data-toggle="modal" data-target="#modal-delete"><i class="dw dw-delete-3"></i> Eliminar</button>
                                </div>
                            </div>
                        </td>
                    </tr>
...
<script>
    ...
    $("#deleted").click(function() {
        ...
        if (id_deleted > 0) {
            $.ajax({
                url: "/gestion-reservas/huespedes/eliminar",
                type: "POST",
                data: {
                ...
                success: function(resp) {
                    ...
                    if (resp == 'ok') { // SI SE ELIMINA CLIENTE
                        ...
                        toastr["success"](`Huésped Eliminado Correctamente`, "Gestión de Mis Huéspedes")
