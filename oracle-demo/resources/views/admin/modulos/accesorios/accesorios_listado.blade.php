    ...
    <div class="card-box mb-30">
        ...
        <div class="pb-20 table-responsive">
            <table class="data-table table stripe hover nowrap text-center" id="tabla">
                <thead>
                    <tr>
                        <th class="table-plus datatable-nosort">Nombre de Accesorio</th>
                        <th>Estado</th>
                ...
                <tbody>
                    ...
                    <tr id="fila_<?= $accesorio->id_accesorio ?>">
                        <td class="table-plus"><input type="hidden" value="<?= $accesorio->accesorio ?>" id="nombre_<?= $accesorio->id_accesorio ?>">{{$accesorio->accesorio}}</td>
                        <td class="text-center"><?= $accesorio->estado == 'Y' ? "<span class='badge badge-success'>Activo</span>" : "<span class='badge badge-danger'>Inactivo</span>" ?></td>
                        <td>{{ordenar_fechaHoraMinutoHumano($accesorio->created_at) }}</td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                    <a class="dropdown-item" href="/dashboard/accesorios/{{$accesorio->id_accesorio}}/editar"><i class="dw dw-edit2"></i> Editar</a>
                                    @if(auth()->user()->id_rol <= 2)
                                    <button type="button" id="<?= $accesorio->id_accesorio ?>" class="dropdown-item btn_deleted" data-toggle="modal" data-target="#modal-delete"><i class="dw dw-delete-3"></i> Eliminar</button>
                                    @endif
                                </div>
                            </div>
                        </td>
...
<script>
    ...
    $("#deleted").click(function() {
        ...
        if (id_deleted > 0) {
            $.ajax({
                url: "/dashboard/accesorios/eliminar",
                type: "POST",
                data: {
                ...
                success: function(resp) {
                    ...
                    if (resp == 'ok') { // SI SE ELIMINA CLIENTE
                        ...
                        toastr["success"](`Accesorio Eliminado Correctamente`, "Gestión de Accesorios")
