        ...
        <div class="pb-20 table-responsive">
            <table class="data-table table stripe hover nowrap text-center" id="tabla">
                ...
                <tbody>
                    ...
                    <tr id="fila_<?= $cliente->id_cliente ?>">
                        <td class="table-plus">
                            <div class="text-left ml-4"><input type="hidden" value="<?= $cliente->nombre . ' ' . $cliente->apellido ?>" id="nombre_<?= $cliente->id_cliente ?>">
                                <b>Nombre: </b>{{$cliente->nombre . ' ' . $cliente->apellido }}<br>
                                <b>DNI: </b>{{!empty($cliente->dni) ? $cliente->dni : 'Sin información'}}<br>
                                <b>Celular: </b>{{!empty($cliente->celular) ? $cliente->celular : '-'}}<br>
                                <b>Email: </b>{{!empty($cliente->email) ? $cliente->email : '-'}}
                            </div>
                        </td>
                        <td>{{ $cliente->reservas > 0 ? formatear_miles($cliente->reservas) : 0}}</td>
                        <td class="text-center"><?= $cliente->estado == 'Y' ? "<span class='badge badge-success'>Activo</span>" : "<span class='badge badge-danger'>Inactivo</span>" ?></td>
                        <td>{{ordenar_fechaHoraMinutoHumano($cliente->created_at) }}</td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                    <a class="dropdown-item" href="/dashboard/clientes/{{$cliente->id_cliente}}/<?= $cliente->estado == 'Y' ? 'desactivar' : 'activar' ?>"><i class="dw dw-edit2"></i> <?= $cliente->estado == 'Y' ? 'Desactivar' : 'Activar' ?></a>
                                    @if(auth()->user()->id_rol <= 2) <button type="button" id="<?= $cliente->id_cliente ?>" class="dropdown-item btn_deleted" data-toggle="modal" data-target="#modal-delete"><i class="dw dw-delete-3"></i> Eliminar</button>
                                        @endif
                                </div>
                            </div>
                        </td>
...
<script>
    ...
    $("#deleted").click(function() {
        ...
        if (id_deleted > 0) {
            $.ajax({
                url: "/dashboard/clientes/eliminar",
                type: "POST",
                data: {
                ...
                success: function(resp) {
                    ...
                    if (resp == 'ok') { // SI SE ELIMINA CLIENTE
                        ...
                        toastr["success"](`Cliente Eliminado Correctamente`, "Gestión de Clientes")
                ...
                error: function(error) {
                    Swal.close();
                    toastr["error"](`Ha ocurrido un error al eliminar registro. Recargue e intente nuevamente.`, "Error Interno")
