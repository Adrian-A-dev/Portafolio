    ...
    <div class="card-box mb-30">
        ...
        <div class="pb-20 table-responsive">
            <table class="data-table table stripe hover nowrap text-center" id="tabla">
                <thead>
                    <tr>
                        <th class="table-plus datatable-nosort">Tipo de Multa</th>
                        <th>Estado</th>
                ...
                <tbody>
                    ...
                    <tr id="fila_<?= $tipo_multa->id_tipo_multa ?>">
                        <td class="table-plus"><input type="hidden" value="<?= $tipo_multa->tipo_multa ?>" id="nombre_<?= $tipo_multa->id_tipo_multa ?>">{{$tipo_multa->tipo_multa}}</td>
                        <td class="text-center"><?= $tipo_multa->estado == 'Y' ? "<span class='badge badge-success'>Activo</span>" : "<span class='badge badge-danger'>Inactivo</span>" ?></td>
                        <td>{{ordenar_fechaHoraMinutoHumano($tipo_multa->created_at) }}</td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                    <a class="dropdown-item" href="/dashboard/tipo-multas/{{$tipo_multa->id_tipo_multa}}/editar"><i class="dw dw-edit2"></i> Editar</a>
                                    @if(auth()->user()->id_rol <= 2)
                                    <button type="button" id="<?= $tipo_multa->id_tipo_multa ?>" class="dropdown-item btn_deleted" data-toggle="modal" data-target="#modal-delete"><i class="dw dw-delete-3"></i> Eliminar</button>
                                    @endif
                                </div>
                            </div>
                        </td>
...
<script>
    ...
    $("#deleted").click(function() {
        ...
        if (id_deleted > 0) {
            $.ajax({
                url: "/dashboard/tipo-multas/eliminar",
                type: "POST",
                data: {
                ...
                success: function(resp) {
                    ...
                    if (resp == 'ok') { // SI SE ELIMINA CLIENTE
                        ...
                        toastr["success"](`Tipo de Multa Eliminada Correctamente`, "Gestión de Tipo de Multas")
