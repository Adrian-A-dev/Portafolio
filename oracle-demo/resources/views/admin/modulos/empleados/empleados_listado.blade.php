        ...
        <div class="pb-20 table-responsive">
            <table class="data-table table stripe hover nowrap text-center" id="tabla">
                ...
                <tbody>
                    ...
                    <tr id="fila_<?= $empleado->id_empleado ?>">
                        <td class="table-plus text-left"><input type="hidden" value="<?= $empleado->nombre.' '.$empleado->apellido ?>" id="nombre_<?= $empleado->id_empleado ?>">
                            <b class="ml-4">Nombre:</b>{{$empleado->nombre.' '.$empleado->apellido}} <br>
                            <b class="ml-4">Cargo:</b> <?= !empty($empleado->cargo->cargo) ? $empleado->cargo->cargo : "<span class='text-danger'>SIN INFORMACIÓN</span>" ?><br>
                            <b class="ml-4">Email:</b> {{$empleado->email}}<br>
                            <b class="ml-4">Celular:</b> {{!empty($empleado->celular) ? $empleado->celular : '-'}}
                        </td>
                        <td class="text-left">
                            <b class="ml-4">Usuario: </b> {{$empleado->usuario->username}}<br>
                            <b class="ml-4">Rol: </b> {{$empleado->usuario->rol->rol}}<br>
                            <b class="ml-4">Última Conexión: </b> <?= !empty($empleado->usuario->ultimo_acceso) ? ordenar_fechaHoraHumano($empleado->usuario->ultimo_acceso) : "<span class='text-danger'>SIN INFORMACIÓN</span>" ?>
                        </td>
                        <td class="text-center"><?= $empleado->estado == 'Y' ? "<span class='badge badge-success'>Activo</span>" : "<span class='badge badge-danger'>Inactivo</span>" ?></td>
                        <td>{{ordenar_fechaHoraMinutoHumano($empleado->created_at) }}</td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                    <a class="dropdown-item" href="/dashboard/empleados/{{$empleado->id_empleado}}/editar"><i class="dw dw-edit2"></i> Editar</a>
                                    @if(auth()->user()->id_rol <= 2)
                                    <button type="button" id="<?= $empleado->id_empleado ?>" class="dropdown-item btn_deleted" data-toggle="modal" data-target="#modal-delete"><i class="dw dw-delete-3"></i> Eliminar</button>
                                    @endif
                                </div>
                            </div>
                        </td>
...
<script>
    ...
    $("#deleted").click(function() {
        ...
        if (id_deleted > 0) {
            $.ajax({
                url: "/dashboard/empleados/eliminar",
                type: "POST",
                data: {
                ...
                success: function(resp) {
                    ...
                    if (resp == 'ok') { // SI SE ELIMINA CLIENTE
                        ...
                        toastr["success"](`Empleado Eliminado Correctamente`, "Gestión de Empleados")
