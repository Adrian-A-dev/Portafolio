    ...
    <div class="card-box mb-30">
        ...
        <div class="pb-20 table-responsive">
            <table class="data-table table stripe hover nowrap text-center" id="tabla">
                <thead>
                    <tr>
                        <th class="table-plus datatable-nosort">Multa</th>
                        <th>Tipo de Multa</th>
                        <th>Valor</th>
                        <th>Estado</th>
                ...
                <tbody>
                    ...
                    <tr id="fila_<?= $multa->id_multa ?>">
                        <td class="table-plus"><input type="hidden" value="<?= $multa->multa ?>" id="nombre_<?= $multa->id_multa ?>">{{$multa->multa}}</td>
                        <td class="table-plus">{{!empty($multa->tipo_multa->tipo_multa) ? strtoupper($multa->tipo_multa->tipo_multa) : 'Sin información'}}</td>
                        <td class="table-plus">{{$multa->monto > 0 ? formatear_numero($multa->monto) : '$0'}}</td>
                        <td class="text-center"><?= $multa->estado == 'Y' ? "<span class='badge badge-success'>Activo</span>" : "<span class='badge badge-danger'>Inactivo</span>" ?></td>
                        <td>{{ordenar_fechaHoraMinutoHumano($multa->created_at) }}</td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                    <a class="dropdown-item" href="/dashboard/multas/{{$multa->id_multa}}/editar"><i class="dw dw-edit2"></i> Editar</a>
                                    @if(auth()->user()->id_rol <= 2)
                                    <button type="button" id="<?= $multa->id_multa ?>" class="dropdown-item btn_deleted" data-toggle="modal" data-target="#modal-delete"><i class="dw dw-delete-3"></i> Eliminar</button>
                                    @endif
                                </div>
                            </div>
                        </td>
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp == 'ok') { // SI SE ELIMINA CLIENTE
                        ...
                        toastr["success"](`Multa Eliminada Correctamente`, "Gestión de Multas")
